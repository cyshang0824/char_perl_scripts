...
# 20240922	add IV_Curve Function
# 20241010	modify sch vilh from V1.8 to 1.71
# History:
# 20230930 change to only swing VDD_I3C to prevent 105C will stress device. And 1V and 0.995V looks not work well(cannot find bourdary at config 63/65 SCH_VILH)
# 20231009 1. fixed hpti -ret_len bugs and this caused the device might be hurt. example: hpti -90000 <cmd>
#          2. $gScan_port = "jtag_tdo_p";
# 20231108 1. fixed leakage measurement
	# For the 1.8V operating mode (disable18_mode):

					# Set I3C to 1.71V and then force voltage on pin to 0V and 1.71V and measure pin leakage at each force condition.
					# Set I3C to 1.80V and then force voltage on pin to 0V and 1.80V and measure pin leakage at each force condition.
					# Set I3C to 1.89V and then force voltage on pin to 0V and 1.89V and measure pin leakage at each force condition.

					# The measurement done at 0V should be labelled IIL.
					# The measurement done when the pin is forced to VDD should be labelled IIH.

	# For the 1.0V operating mode (disable10_mode):

					# Set I3C to 0.95V and then force voltage on pin to 0V and 0.95V and 3.63V and measure pin leakage at each force condition.
					# Set I3C to 1.00V and then force voltage on pin to 0V and 1.00V and 3.63V and measure pin leakage at each force condition.
					# Set I3C to 1.05V and then force voltage to pin to 0V and 1.05V and 3.63V and measure pin leakage at each force condition.

					# The measurement done at 0V should be labelled IIL.
					# The measurement done when the pin is forced to VDD should be labelled IIH.
					# The measurement done when the pin is forced to 3.63V should be labelled IIH_3p63.
########################################################
...
$sch_vilh_dataset1 = "SPRM 16,1,$lvl,$dps";
$sch_vilh_dataset2 = "SPRM 1,1,$lvl,$dps";
...
sub vilh_sweep_linear_vhyst {
  ...
  if ($vil eq "tst") {	     # input low being tested	 VIL
	    ...
	    while (($current_put_mv <= $limit_2) && ($pf_flag_fine == 0)) {             #when completing fine search then stop
			 if (($pf_flag_coarse == 0)) {$current_put_mv = $current_put_mv + 50 ; }#30mv per step for coarse search
		     if (($pf_flag_coarse == 1)) {print "***** start pf_flag_coarse search *****\n";$current_put_mv = $current_put_mv + 10 ;} #5mv per step for fine search
			 ...
			 if (($pf_flag_coarse == 1)&&($back_coarse_volt == 0)) {$current_put_mv = $coarse_volt - 50 ; $back_coarse_volt = 1;} #back the voltage of coarse search
...
sub sch_vil {
	...
	if (($pat_cfg eq "55")||($pat_cfg eq "56")||($pat_cfg eq "57")||($pat_cfg eq "58")) 
		{ 	$vil_val = &vilh_sweep_linear_vhyst($put,300,1300,"tst",$init_vih_mv,$pat_cfg,$mode);}
    else{	$vil_val = &vilh_sweep_linear_vhyst($put,300,1300,"tst",$init_vih_mv,$pat_cfg,$mode);}
...
sub sch_vih {
	...
	if (($pat_cfg eq "55")||($pat_cfg eq "56")||($pat_cfg eq "57")||($pat_cfg eq "58"))	
	{	$vih_val = &vilh_sweep_linear_vhyst($put,1300,300,$init_vil_mv,"tst",$pat_cfg,$mode);	}
	else
	{	$vih_val = &vilh_sweep_linear_vhyst($put,1300,300,$init_vil_mv,"tst",$pat_cfg,$mode);	}
...
if (($test_mode eq "typ") || ($test_mode eq "rep") || ($test_mode eq "typ_1v8")) {
	...
	if ($test_mode eq "typ_1v8"){	@vdd_pwr = ("typ_1v8");		}
	...
	die "Incorrect test mode selected -> typ(typ_1v8)/rep/pvt\n";
}
...
foreach $temp (@temp_C) {
	...
	while ($current_count < $rep) {	
        ...
        if (($test eq "all") || ($test eq "sch_vilh") || ($test eq "vilh_all")) {
		    ...
		    foreach $vdd_v (@vdd_pwr) {
				...
				for (my $i3c_scan_V = 1.81; $i3c_scan_V >= 1.67; $i3c_scan_V -= 0.02)
				{
					...
					{		 
						...
						`hpti '$sch_vilh_dataset1'`;	
						print "$sch_vilh_dataset1\n";
						`hpti 'SQSL "i3c_DC_cfg_$pat_cfg"'`; `hpti 'FTST?'`;
						print "-- i3c_DC_cfg_$pat_cfg\n";
						...
						foreach $put (@i3c_put) {
							# if (($pat_cfg eq "43")||($pat_cfg eq "45")||($pat_cfg eq "47")||($pat_cfg eq "49")||($pat_cfg eq "51")||($pat_cfg eq "53")||($pat_cfg eq "55")||($pat_cfg eq "57")||($pat_cfg eq "59")||($pat_cfg eq "61")||($pat_cfg eq "63")||($pat_cfg eq "65")) {
							if ($pat_cfg_execute==1) {
									&sch_vih($vdd_v,$put,$mode,$retval_array2[1],$retval_array1[1],HtoL,0,$force_vih,$pat_cfg,$mode);
									&sch_vil($vdd_v,$put,$mode,$retval_array2[1],$retval_array1[1],LtoH,0,$force_vih,$pat_cfg,$mode);
							}
						}											
					}
				}
		...
		if (($test eq "all") || ($test eq "iolh")) {
            ...
            foreach $vdd_v (@vdd_pwr) {
				...
				print "vdd_v setting:  $vdd_v\n";
            	foreach $pat_cfg (@pat_label) {		 
			       # foreach $vddo_v (@i3c_pwr_tst) {
				   $cfg_dataset = "SPRM $atp_wfs,$atp_tim,$lvl,$dps";	   
			       #$cfg_pattern = "i3c_DC_cfg_$pat_cfg";
				   `hpti '$cfg_dataset'`;	
                   ...
                   print ("vdd_i3c setting $retval_array2[1] \n");		
					   ...
					   $FTST_val = `hpti 'FTST?'`;
		               #print "FTST setting: SQSL $iolh_pattern, $FTST_val\n";		
