...
my $pin_i=0;
	...
	&pupd_return_val_datalog($ret_val_string,$$vin_test,$vdd_gpio,$vddc);
...
{
	($ret_val_string,$vin_test,$vdd_gpio,$vddc) = @_;
	@ret_line=();
...
#			print "measure_value=$measure_value[$i]\n";
			...
			{	@temp_array=();
				...
				foreach $put (@temp_array)
				{
					...
					print "$put current $pin_i uA is measured, Rpu/pd = $R_value1 kohm, $Irng\n";				
					print fh_pupd ("$current_count,$rep,$split,$device,$test_mode,$temp,$cal_reference,$Rpd_cfg,$mode,$vdd_gpio,$vddc,");
					print fh_pupd ("$put,$parameter,$vin_test,$lo_limit,$hi_limit,$pin_i,$Irng,$R_value1\n");
 				}
			}
			else
			{			#major run here
...
#			print "measure_value=$measure_value[$i]\n";
			...
			{	@temp_array=();
				...
				foreach $put (@temp_array) {
					...
					print "$put -> $iolh_current mA\n";
					$meas_i = abs($iolh_current);
					# print "IOLH value for $put = $meas_i mA \@$Vin_set mV\n";
					print fh_iolh ("$current_count,$rep,$split,$device,$test_mode,$temp,$cal_reference,iolh_cfg_$iolh_cfg,$mode,$vdd_gpio,$vddc,");
					print fh_iolh ("$put,$Vin_set,$meas_i\n");
...
sub lvcmos_leak {
    ...
    ($vddo_v,$Vin_set,$put_i,$mode,$vdd_gpio,$vddc,$parameter,$leak_cfg) = @_;
	...
	$ret_val_string = `hpti 'PMUR? VAL,($put_i)'`;
...
#			print "measure_value=$measure_value[$i]\n";
			...
			{	@temp_array=();
				...
				{
					$leak_current=$measure_value_string;
					print "$put  -> $Vin_set mv for $put_i = $meas_i uA\n";
					print fh_leak ("$current_count,$rep,$split,$device,$test_mode,$temp,$cal_reference,$leak_cfg,$mode,$vdd_gpio,$vddc,");
					print fh_leak ("$put_i,$parameter,$Vin_set,$meas_i\n");
				}
			}
			else
			{			# major run here
				...
				foreach $vdd_v (@vdd_pwr)
				{
					...
					{			
						...
						foreach $Rpd_cfg (@Rpd_label) 
						{
							...
							$put=join(",", @lvcmos_all_put);
							$Irange_Fixed="Yes";
							&PUPD_I_in_parallel($vdd_v,$term_v,$put,$mode,$retval_array2[1],$retval_array1[1],$parameter,"IRB",$Rpd_cfg,$Irange_Fixed);
							`hpti 'RLYC AC,OFF,($put)'`;  	
						}
				}
			}
