...
sub calibrate_temp {
	...
	$case_temp=$atc_array[1];

	print "now temp=$atc_array[1]\n";
	$temp_diff=abs($cal_temp-$case_temp);
	...
	if ($temp_diff > 5)
	{
		...
		$case_temp=`/proj/me_proj/cyshang/ATC_control/atc_ps1600/set_$tester_name\_p$atc_port.prl $cal_temp $soak $si_therm`;
		chomp($case_temp);
		print " \n -- temp ready at $case_temp C \n";
		print " \n -- temp ready at $case_temp C \n";
		print " \n -- temp ready at $case_temp C \n";
	}	
	else
			...
			{	
				...
				foreach $put (@temp_array) {
					...
					{	print "$put  -> $volh_volt mV \@$iforce mA \@min current\n";	}
						...
						print fh_volh ("$current_count,$rep,$split,$device,$test_mode,$temp,$iolh_cfg,$mode,$vdd_gpio,$vddc,");
