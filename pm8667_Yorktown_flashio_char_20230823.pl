...
# History:
# 20230814: modified the formula for LTT(double Vref - Vref)/measured current and change pd supply voltage to GND
# 20230816: fixed UTOT to UTOT 1,,6
# 20230823: merge data and CA pins
########################################################
...
@flash_cau_od_vec = ("flash_io_2400_test_1_pu_ctt","flash_io_2400_test_2_pu_ctt", "flash_io_2400_test_5_pu_ltt", "flash_io_2400_test_6_pu_ltt");
@flash_dxu_od_vec = ("flash_io_2400_test_1_pu_ctt","flash_io_2400_test_2_pu_ctt", "flash_io_2400_test_5_pu_ltt", "flash_io_2400_test_6_pu_ltt");
@flash_dxd_od_vec = ("flash_io_2400_test_3_pd_ctt","flash_io_2400_test_4_pd_ctt", "flash_io_2400_test_7_pd_ltt", "flash_io_2400_test_8_pd_ltt");
@flash_cad_od_vec = ("flash_io_2400_test_3_pd_ctt","flash_io_2400_test_4_pd_ctt", "flash_io_2400_test_7_pd_ltt", "flash_io_2400_test_8_pd_ltt");
...
#pvt_comp_drvpu_od($flash_mode,$flash_test_vectors,$current_flash_sply_mv,$current_vref_mv);
	...
	`hpti 'PSLV $atp_dps,$supply_v,$pwr_loads{$flash_supply_name},LOZ,$pwr_seq{$flash_supply_name},($flash_supply_name)'`;
	`hpti 'PSLV $atp_dps,$vref_v,$pwr_loads{$flash_vref_name},LOZ,$pwr_seq{$flash_vref_name},($flash_vref_name)'`;
	
	print "PSLV $atp_dps,$supply_v,$pwr_loads{$flash_supply_name},LOZ,$pwr_seq{$flash_supply_name},($flash_supply_name)\n";
	print "PSLV $atp_dps,$vref_v,$pwr_loads{$flash_vref_name},LOZ,$pwr_seq{$flash_vref_name},($flash_vref_name)\n";
	...
	foreach $put (@flash_put) {
		...
		if (($flash_pin_class{$put} eq "flash_cmd") && ($current_put_type eq "ca")) {
			if ($number <= 5){
				if ($flash_put_ca1 ne ""){$flash_put_ca1=$flash_put_ca1 . "," . $put;	}
				else{$flash_put_ca1= $put;}
			}
			elsif ($number > 5 && $number <= 10){
				if ($flash_put_ca2 ne ""){	$flash_put_ca2=$flash_put_ca2 . "," . $put;	}
				else{$flash_put_ca2= $put;}
			}
			elsif ($number > 10 && $number <= 15){
				if ($flash_put_ca3 ne ""){	$flash_put_ca3=$flash_put_ca3 . "," . $put;	}
				else{$flash_put_ca3= $put;}
			}
				...
				if ($number <= 5){	
					if ($flash_put_dx1 ne ""){$flash_put_dx1=$flash_put_dx1 . "," . $put;	}
					else{$flash_put_dx1= $put;}
				}
				elsif ($number > 5 && $number <= 10){	
					if ($flash_put_dx2 ne ""){$flash_put_dx2=$flash_put_dx2 . "," . $put;	}
					else{$flash_put_dx2= $put;}
				}
				elsif ($number > 10 && $number <= 15){	
					if ($flash_put_dx3 ne ""){$flash_put_dx3=$flash_put_dx3 . "," . $put;	}
					else{$flash_put_dx3= $put;}
				}
			}
		...
		{
			print "run dx1!\n";
			#*************  dxpin need to split to two groups and "hpti 'RLYC AC,OFF,(@)", otherwise the measured values will be incorrected 25ohms become 35ohms **************
			# I think it is related to ppmu power capbility
			...
			&od_return_val_datalog($ret_val,$vref_mv,$force_pad_v*1000);
			...
			if ($flash_put_dx3 ne ""){
				print "run dx3!\n";
				$ret_val="";			
				$ret_val = &pin_forceV_measI($flash_put_dx3,$force_pad_v,-40000,40000,"IRD");
				&od_return_val_datalog($ret_val,$vref_mv,$force_pad_v*1000);
			}
		}
		if ( $current_put_type eq "ca")
		{
			...
			$pin_class="addr_cmd";
			$ret_val="";
			$ret_val = &pin_forceV_measI($flash_put_ca1,$force_pad_v,-40000,40000,"IRD");
			&od_return_val_datalog($ret_val,$vref_mv,$force_pad_v*1000);
			...
			$pin_class="addr_cmd";
			$ret_val="";
			$ret_val = &pin_forceV_measI($flash_put_ca2,$force_pad_v,-40000,40000,"IRD");
			&od_return_val_datalog($ret_val,$vref_mv,$force_pad_v*1000);
			...
			$pin_class="addr_cmd";
			$ret_val="";
			$ret_val = &pin_forceV_measI($flash_put_ca3,$force_pad_v,-40000,40000,"IRD");
			&od_return_val_datalog($ret_val,$vref_mv,$force_pad_v*1000);			
...
sub od_return_val_datalog{
	my($ret_val_string,$vref_mv,$force_pad_mv) = @_;
	@ret_line=();
	@ret_line=split (/PMUR/,$ret_val_string);
	chomp(@ret_line);
	foreach $line_string (@ret_line) {
...
#			print "measure_value=$measure_value[$i]\n";
			...
			{	@temp_array=();
				...
				foreach $put (@temp_array) {
					...
					if ($ipad_pupd_ma != 0)	{$rocd_pupd = ($vref_mv*2 - $force_pad_mv)/($ipad_pupd_ma);} else { $rocd_pupd="NA";}
					print "pin $put  Force $force_pad_mv mV results in $ipad_pupd_ma mA and $rocd_pupd ohms\n";
...
if (($test_mode eq "typ") || ($test_mode eq "rep")) {
	...
	if ($test_mode eq "typ") {
		@flash_put = (
			"fc00_ale_p","fc00_ce0_n","fc00_ce1_n","fc00_ce2_n","fc00_ce3_n","fc00_ce4_n","fc00_ce5_n","fc00_ce6_n","fc00_ce7_n","fc00_cle_p","fc00_dbi_p","fc00_odt_n","fc00_re_nc","fc00_re_nt","fc00_we_n","fc01_ale_p","fc01_ce0_n","fc01_ce1_n","fc01_ce2_n","fc01_ce3_n","fc01_ce4_n","fc01_ce5_n","fc01_ce6_n","fc01_ce7_n","fc01_cle_p","fc01_dbi_p","fc01_odt_n","fc01_re_nc","fc01_re_nt","fc01_we_n","fc02_ale_p","fc02_ce0_n","fc02_ce1_n","fc02_ce2_n","fc02_ce3_n","fc02_ce4_n","fc02_ce5_n","fc02_ce6_n","fc02_ce7_n","fc02_cle_p","fc02_dbi_p","fc02_odt_n","fc02_re_nc","fc02_re_nt","fc02_we_n","fc03_ale_p","fc03_ce0_n","fc03_ce1_n","fc03_ce2_n","fc03_ce3_n","fc03_ce4_n","fc03_ce5_n","fc03_ce6_n","fc03_ce7_n","fc03_cle_p","fc03_dbi_p","fc03_odt_n","fc03_re_nc","fc03_re_nt","fc03_we_n","fc04_ale_p","fc04_ce0_n","fc04_ce1_n","fc04_ce2_n","fc04_ce3_n","fc04_ce4_n","fc04_ce5_n","fc04_ce6_n","fc04_ce7_n","fc04_cle_p","fc04_dbi_p","fc04_odt_n","fc04_re_nc","fc04_re_nt","fc04_we_n","fc05_ale_p","fc05_ce0_n","fc05_ce1_n","fc05_ce2_n","fc05_ce3_n","fc05_ce4_n","fc05_ce5_n","fc05_ce6_n","fc05_ce7_n","fc05_cle_p","fc05_dbi_p","fc05_odt_n","fc05_re_nc","fc05_re_nt","fc05_we_n","fc06_ale_p","fc06_ce0_n","fc06_ce1_n","fc06_ce2_n","fc06_ce3_n","fc06_ce4_n","fc06_ce5_n","fc06_ce6_n","fc06_ce7_n","fc06_cle_p","fc06_dbi_p","fc06_odt_n","fc06_re_nc","fc06_re_nt","fc06_we_n","fc07_ale_p","fc07_ce0_n","fc07_ce1_n","fc07_ce2_n","fc07_ce3_n","fc07_ce4_n","fc07_ce5_n","fc07_ce6_n","fc07_ce7_n","fc07_cle_p","fc07_dbi_p","fc07_odt_n","fc07_re_nc","fc07_re_nt","fc07_we_n","fc08_ale_p","fc08_ce0_n","fc08_ce1_n","fc08_ce2_n","fc08_ce3_n","fc08_ce4_n","fc08_ce5_n","fc08_ce6_n","fc08_ce7_n","fc08_cle_p","fc08_dbi_p","fc08_odt_n","fc08_re_nc","fc08_re_nt","fc08_we_n","fc09_ale_p","fc09_ce0_n","fc09_ce1_n","fc09_ce2_n","fc09_ce3_n","fc09_ce4_n","fc09_ce5_n","fc09_ce6_n","fc09_ce7_n","fc09_cle_p","fc09_dbi_p","fc09_odt_n","fc09_re_nc","fc09_re_nt","fc09_we_n","fc10_ale_p","fc10_ce0_n","fc10_ce1_n","fc10_ce2_n","fc10_ce3_n","fc10_ce4_n","fc10_ce5_n","fc10_ce6_n","fc10_ce7_n","fc10_cle_p","fc10_dbi_p","fc10_odt_n","fc10_re_nc","fc10_re_nt","fc10_we_n","fc11_ale_p","fc11_ce0_n","fc11_ce1_n","fc11_ce2_n","fc11_ce3_n","fc11_ce4_n","fc11_ce5_n","fc11_ce6_n","fc11_ce7_n","fc11_cle_p","fc11_dbi_p","fc11_odt_n","fc11_re_nc","fc11_re_nt","fc11_we_n","fc12_ale_p","fc12_ce0_n","fc12_ce1_n","fc12_ce2_n","fc12_ce3_n","fc12_ce4_n","fc12_ce5_n","fc12_ce6_n","fc12_ce7_n","fc12_cle_p","fc12_dbi_p","fc12_odt_n","fc12_re_nc","fc12_re_nt","fc12_we_n","fc13_ale_p","fc13_ce0_n","fc13_ce1_n","fc13_ce2_n","fc13_ce3_n","fc13_ce4_n","fc13_ce5_n","fc13_ce6_n","fc13_ce7_n","fc13_cle_p","fc13_dbi_p","fc13_odt_n","fc13_re_nc","fc13_re_nt","fc13_we_n","fc14_ale_p","fc14_ce0_n","fc14_ce1_n","fc14_ce2_n","fc14_ce3_n","fc14_ce4_n","fc14_ce5_n","fc14_ce6_n","fc14_ce7_n","fc14_cle_p","fc14_dbi_p","fc14_odt_n","fc14_re_nc","fc14_re_nt","fc14_we_n","fc15_ale_p","fc15_ce0_n","fc15_ce1_n","fc15_ce2_n","fc15_ce3_n","fc15_ce4_n","fc15_ce5_n","fc15_ce6_n","fc15_ce7_n","fc15_cle_p","fc15_dbi_p","fc15_odt_n","fc15_re_nc","fc15_re_nt","fc15_we_n","fc00_dq0_p","fc00_dq1_p","fc00_dq2_p","fc00_dq3_p","fc00_dq4_p","fc00_dq5_p","fc00_dq6_p","fc00_dq7_p","fc00_dqs_pc","fc00_dqs_pt","fc01_dq0_p","fc01_dq1_p","fc01_dq2_p","fc01_dq3_p","fc01_dq4_p","fc01_dq5_p","fc01_dq6_p","fc01_dq7_p","fc01_dqs_pc","fc01_dqs_pt","fc02_dq0_p","fc02_dq1_p","fc02_dq2_p","fc02_dq3_p","fc02_dq4_p","fc02_dq5_p","fc02_dq6_p","fc02_dq7_p","fc02_dqs_pc","fc02_dqs_pt","fc03_dq0_p","fc03_dq1_p","fc03_dq2_p","fc03_dq3_p","fc03_dq4_p","fc03_dq5_p","fc03_dq6_p","fc03_dq7_p","fc03_dqs_pc","fc03_dqs_pt","fc04_dq0_p","fc04_dq1_p","fc04_dq2_p","fc04_dq3_p","fc04_dq4_p","fc04_dq5_p","fc04_dq6_p","fc04_dq7_p","fc04_dqs_pc","fc04_dqs_pt","fc05_dq0_p","fc05_dq1_p","fc05_dq2_p","fc05_dq3_p","fc05_dq4_p","fc05_dq5_p","fc05_dq6_p","fc05_dq7_p","fc05_dqs_pc","fc05_dqs_pt","fc06_dq0_p","fc06_dq1_p","fc06_dq2_p","fc06_dq3_p","fc06_dq4_p","fc06_dq5_p","fc06_dq6_p","fc06_dq7_p","fc06_dqs_pc","fc06_dqs_pt","fc07_dq0_p","fc07_dq1_p","fc07_dq2_p","fc07_dq3_p","fc07_dq4_p","fc07_dq5_p","fc07_dq6_p","fc07_dq7_p","fc07_dqs_pc","fc07_dqs_pt","fc08_dq0_p","fc08_dq1_p","fc08_dq2_p","fc08_dq3_p","fc08_dq4_p","fc08_dq5_p","fc08_dq6_p","fc08_dq7_p","fc08_dqs_pc","fc08_dqs_pt","fc09_dq0_p","fc09_dq1_p","fc09_dq2_p","fc09_dq3_p","fc09_dq4_p","fc09_dq5_p","fc09_dq6_p","fc09_dq7_p","fc09_dqs_pc","fc09_dqs_pt","fc10_dq0_p","fc10_dq1_p","fc10_dq2_p","fc10_dq3_p","fc10_dq4_p","fc10_dq5_p","fc10_dq6_p","fc10_dq7_p","fc10_dqs_pc","fc10_dqs_pt","fc11_dq0_p","fc11_dq1_p","fc11_dq2_p","fc11_dq3_p","fc11_dq4_p","fc11_dq5_p","fc11_dq6_p","fc11_dq7_p","fc11_dqs_pc","fc11_dqs_pt","fc12_dq0_p","fc12_dq1_p","fc12_dq2_p","fc12_dq3_p","fc12_dq4_p","fc12_dq5_p","fc12_dq6_p","fc12_dq7_p","fc12_dqs_pc","fc12_dqs_pt","fc13_dq0_p","fc13_dq1_p","fc13_dq2_p","fc13_dq3_p","fc13_dq4_p","fc13_dq5_p","fc13_dq6_p","fc13_dq7_p","fc13_dqs_pc","fc13_dqs_pt","fc14_dq0_p","fc14_dq1_p","fc14_dq2_p","fc14_dq3_p","fc14_dq4_p","fc14_dq5_p","fc14_dq6_p","fc14_dq7_p","fc14_dqs_pc","fc14_dqs_pt","fc15_dq0_p","fc15_dq1_p","fc15_dq2_p","fc15_dq3_p","fc15_dq4_p","fc15_dq5_p","fc15_dq6_p","fc15_dq7_p","fc15_dqs_pc","fc15_dqs_pt"
			);
		# @flash_put_odt = (
...
if ($test eq "all") {
	...
	$datalog_file_leak = "flashio_leak_datalog_${split}_${test_mode}_${date_array[1]}${date_array[2]}.csv";
	open fh_leak, ">>$datalog_file_leak";
	print "IO Leak Data will be written to -> $datalog_file_leak\n";
	
	$datalog_file_vilh = "flashio_vilh_datalog_${split}_${test_mode}_${date_array[1]}${date_array[2]}.csv";
	open fh_vilh, ">>$datalog_file_vilh";
	print "VILH Data will be written to -> $datalog_file_vilh\n";
	
	$datalog_file_od = "flashio_od_datalog_${split}_${test_mode}_${date_array[1]}${date_array[2]}.csv";
	open fh_od, ">>$datalog_file_od";
	print "PVT compensation Output Drive Data will be written to -> $datalog_file_od\n";
	
	$datalog_file_odt = "flashio_odt_datalog_${split}_${test_mode}_${date_array[1]}${date_array[2]}.csv";
...
#turn on relay to connect flash ZQ to calibration resistor
# UTL1_URW01(UTI102) and UTL1_URW02(UTI103)
 `hpti 'UTOT 1,,6'`;
			...
			if (($flash_select eq "flash") || ($flash_select eq "all")) {
				...
				foreach $current_flash_sply (@flash_io_pwr_tst) {
					...
					foreach $flash_test_vectors (@flash_cad_od_vec) {
						...
						if ($flash_test_vectors =~/ctt/) {					$current_vref_mv = $current_flash_sply_mv * $flash_vref_ratio{"flash_ctt"};	}	
						$current_load_updn="";						
						$current_load_updn= $flash_cad_od{$flash_test_vectors};
						print "\n\n\n\n\nexpected current load = $current_load_updn\n";
						&pvt_comp_drvpupd_od($flash_mode,$flash_test_vectors,$current_flash_sply_mv,$current_vref_mv,"PD");
					}
				}
